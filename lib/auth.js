...
function saveToken ({email, token, sso}) {
  ...
  hosts.forEach(host => {
    ...
    if (sso) netrc.machines[host].sso = sso
  })
  netrc.save()
}
...
function * loginSSO ({save}) {
  ...
  if (save) saveToken({token, email: account.email, sso: url})
...
function * login (options = {}) {
  const Netrc = require('netrc-parser')
  const netrc = new Netrc()
  if (options['sso'] || netrc.machines['api.heroku.com'].sso) {
