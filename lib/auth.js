...
async function loginUserPass ({ save, expires_in: expiresIn }) {
  ...
  try {
    ...
    const secondFactor = await prompt('Two-factor code', { mask: true })
...
async function loginSSO ({ save, browser }) {
  ...
  const token = await prompt('Enter your access token (typing will be hidden)', { hide: true })

  const account = await cli.heroku.get('/account', {
    headers: {
...
async function logout () {
  const token = cli.heroku.options.token
  if (token) {
    ...
    const sessionsP = cli.heroku.delete('/oauth/sessions/~')
      .catch(err => {
    ...
    const defaultAuthorizationP = cli.heroku.get('/oauth/authorizations/~')
      .catch(err => {
    ...
    const authorizationsP = cli.heroku.get('/oauth/authorizations')
      .catch(err => {
        if (err.statusCode === 401 && err.body && err.body.id === 'unauthorized') {
          return []
        }
    ...
    const [, defaultAuthorization, authorizations] = await Promise.all([sessionsP, defaultAuthorizationP, authorizationsP])
