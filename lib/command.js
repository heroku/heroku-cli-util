...
function apiMiddleware (options, preauths, context) {
  const twoFactor = twoFactorWrapper(options, preauths, context)
  return function (response, cb) {
    const warning = response.headers['x-heroku-warning'] || response.headers['warning-message']
...
function heroku (context, options) {
  const host = context.apiUrl || vars.apiUrl || 'https://api.heroku.com'
...
module.exports = function command (options, fn) {
  return function (context) {
    ...
    const handleErr = cli.errorHandler({ debug: context.debug })
    const run = function () {
      context.auth = { password: auth.token() }
      const p = fn(context, heroku(context, options))
      if (!p.catch) return
