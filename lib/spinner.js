...
const color = require('@heroku-cli/color').default
...
class Spinner {
  ...
  _spin () {
    ...
    this.stream.write(this.ansi.cursorUp(this._lines(this._output)))
    const y = this._lines(this.text) - 1
    const lastline = stripAnsi(this.text).split('\n').pop()
    const x = 1 + lastline.length - (this._lines(lastline) - 1) * this._width
