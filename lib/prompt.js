...
function prompt (name, options) {
  ...
  if (options.mask || options.hide) {
    if (!isTTY) {
      let errMsg = `CLI needs to prompt for ${options.name || options.prompt} but stdin is not a tty.`
      if (os.platform() === 'win32') {
        errMsg += ' This is likely a problem with your shell sofware. On win32 plaftorms, we recommend using cmd.exe'
      }
