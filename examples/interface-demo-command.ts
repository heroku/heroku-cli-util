...
export default class InterfaceDemoCommand extends Command {
  ...
  async run(): Promise<void> {
    ...
    ux.stdout(`  ${colors.pipeline('awesome-app-pipeline')}         // Purple, normal weight\n`)
    ux.stdout(`  ${colors.space('production')}                  // Blue, bold with diamond icon\n`)
    ux.stdout(`  ${colors.datastore('postgresql-curly-12345')}  // Yellow, bold with diamond icon\n`)
    ux.stdout(`  ${colors.addon('my-papertrail-addon')}         // Yellow, bold\n`)
